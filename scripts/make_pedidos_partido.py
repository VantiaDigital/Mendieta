...
# -*- coding: utf-8 -*-
"""
Mendieta — Foto de pedidos para el feed: partido Argentina vs Austria.
Mismo molde que pedidos-detalles, con el partido en el copete.

Salidas:
  assets/brand/templates/pedidos-partido-post.png  (1080x1350, feed 4:5)
  + copia en Documentos/Mendieta/partido-austria/FINAL/
"""
import shutil
...
OUT = BASE / "assets" / "brand" / "templates" / "pedidos-partido-post.png"
OUT2 = Path(r"C:/Users/facun/Documentos/Mendieta/partido-austria/FINAL/pedidos-partido-post.png")
...
def divider(d, cy):
    ...
    destello(d, W // 2 - 45, cy, 9, MOSTAZA); destello(d, W // 2 + 45, cy, 9, MOSTAZA)


def step(d, cy, num, lines):
    bx, r = 150, 40
    d.ellipse([bx - r, cy - r, bx + r, cy + r], fill=BORDO)
    fn = mont(44, 700); w, h, oy = tw(d, num, fn)
    d.text((bx - w // 2, cy - h // 2 - oy), num, font=fn, fill=CREMA)
    heights = [tw(d, t, f)[1] for (t, f, _) in lines]; gap = 12
    total = sum(heights) + gap * (len(lines) - 1); y = cy - total // 2; tx = 232
    for (t, f, fill), hh in zip(lines, heights):
        oy = tw(d, t, f)[2]; d.text((tx, y - oy), t, font=f, fill=fill); y += hh + gap
...
def main():
    ...
    cline(d, 250, "ARGENTINA VS AUSTRIA · LUNES 19H", mont(26, 600), BORDO, ls=2)
    cline(d, 296, "PEDÍ PARA EL PARTIDO", font(F_RYE, 60), BORDO)
    cline(d, 392, "Viví el Mundial como en casa", font(F_PLAY, 34), CACAO)
